...
let mesh = createMesh([1, 1, 1, 1, 1, 1, 1, 1, 1], 0)
scene.add(mesh)
...
const sensorDataIDArray = []
querySnapshot.forEach((doc) => {
    sensorData[doc.id] = doc.data().sensors
    sensorDataIDArray.push(doc.id)
})
for (const id of sensorDataIDArray.reverse()) {
    const li = document.createElement("li")
    const date = new Date(id*1000)
    li.innerText = date.toLocaleString('no-NO', {
        timeZone: 'Europe/Oslo'
    })
    li.setAttribute("id", id)
    li.onclick = () => {
        scene.remove(mesh)
        mesh = createMesh(sensorData[id], mesh.rotation.y)
        scene.add(mesh)
    }
